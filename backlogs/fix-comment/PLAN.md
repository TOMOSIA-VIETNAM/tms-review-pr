...
## Task FC1: Scaffold `src/commands/fix-comment.md` — frontmatter + Bước 0-2 (verify + bootstrap)
- Acceptance:
  ...
  - Bootstrap (mục 2 SPEC.md bước 2): file `notebooks/review/<repo>/fix-comment-meta.json` (schema
...
- Acceptance:
  ...
  - 🔴/🟠 → default FIX; agent tự thấy sai/không hợp lý → rẽ theo `decline_needs_confirmation`
    (đọc từ `fix-comment-meta.json`): `true` hỏi dev trước khi quyết decline, `false` tự quyết.
...
- Acceptance:
  ...
  - Reconfigure trigger: dev gõ ý định "đổi cấu hình fix-comment" (hay tương đương) → in giá trị
    hiện tại `fix-comment-meta.json`, hỏi field muốn đổi, ghi ngay.
...
- Dependency: không (độc lập nội dung với FC1-FC5, có thể làm song song) — nhưng nên làm TRƯỚC
  FC5 để khi viết ví dụ/tham chiếu marker trong `fix-comment.md` đã đúng thực tế 2 nơi đều có.

## Task FC7: `CLAUDE.md` — thêm `fix-comment.md` vào bảng cấu trúc + mục kiến trúc
- Acceptance:
  - Bảng cấu trúc (mục "Cấu trúc") thêm dòng `src/commands/fix-comment.md` theo đúng format cột
    hiện có (mô tả ngắn, phân biệt rõ với `review-pr.md`: dev-facing, sửa code thật, không qua
    worktree).
  - Mục "Kiến trúc cốt lõi": thêm đoạn mô tả kiến trúc `fix-comment.md` theo đúng văn phong các
...
## Task FC8: `README.md` / `README.en.md` / `README.ja.md` — giới thiệu `/tms:fix-comment`
- Acceptance:
  - Thêm 1 mục ngắn (đồng bộ cả 3 bản ngôn ngữ, đúng cấu trúc heading tương ứng đã có) giới thiệu
    `/tms:fix-comment`: dùng khi nào (đã có review, muốn fix), khác gì `/tms:review-pr` (dev-facing,
