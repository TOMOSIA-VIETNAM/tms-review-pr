...
`plugins/local`), rồi restart Cursor (hoặc Developer: Reload Window). Command: `/review-pr`.
Sửa `src/` rồi thì chạy lại script + reload.

**Team Marketplace:** admin import URL GitHub repo này (Dashboard → Plugins → Team Marketplaces);
manifest Cursor ở `.cursor-plugin/marketplace.json` (`source: "./src"`).
