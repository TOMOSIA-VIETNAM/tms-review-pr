# Re-review — đồng thuận thread mới + finding cũ đã fix chưa (Bước 6 `review_pr.md`)

Không phải slash command (nằm ngoài `commands/`). `review_pr.md` Bước 6 `Read` file này khi review comments
...
- CHỈ SAU KHI user đồng ý: ghi lesson theo Phần E của `"${CLAUDE_PLUGIN_ROOT}"/src/setup-flow.md`
  (đọc bằng `Read` nếu chưa nạp file này ở Bước 3/4 của `review_pr.md`).
...
2. Trong danh sách comment đã fetch, lọc ra các comment TOP-LEVEL (không phải reply, tức không có
   ...
   của `review_pr.md` (chứa `**Vấn đề**`/`**Issue**`) — đây là các finding do chính lệnh này để lại ở (các)
   lần chạy trước trên PR này.
3. Với MỖI comment như vậy: đối chiếu mô tả vấn đề trong comment với code HIỆN TẠI tại đúng
   path/vùng đó (đã có sẵn trong worktree tạo ở Bước 1 của `review_pr.md`, dùng `Read` tại
   ...
   - **Đã fix** → reply ngắn gọn xác nhận vào ĐÚNG thread đó:
     ...
     Sau đó rẽ theo `auto_resolve_fixed_findings` (đọc từ `meta.json` ở Bước 3 của `review_pr.md`):
