# Re-review — đồng thuận thread mới + finding cũ đã fix chưa (Bước 6 `pr.md`)

Không phải slash command (nằm ngoài `commands/`). `pr.md` Bước 6 `Read` file này khi review comments
...
- CHỈ SAU KHI user đồng ý: ghi lesson theo Phần E của `"${CLAUDE_PLUGIN_ROOT}"/src/setup-flow.md`
  (đọc bằng `Read` nếu chưa nạp file này ở Bước 3/4 của `pr.md`).
...
2. Trong danh sách comment đã fetch, lọc ra các comment TOP-LEVEL (không phải reply, tức không có
   ...
   của `pr.md` (chứa `**Vấn đề**`/`**Issue**`) — đây là các finding do chính lệnh này để lại ở (các)
   lần chạy trước trên PR này.
3. Với MỖI comment như vậy: đối chiếu mô tả vấn đề trong comment với code HIỆN TẠI tại đúng
   path/vùng đó (đã có sẵn trên đĩa từ Bước 1 của `pr.md`, dùng `Read`) — tự phán đoán vấn đề đã
   được fix hay chưa, không có rule cứng, dựa vào đọc hiểu thực tế.
   - **Đã fix** → reply ngắn gọn xác nhận vào ĐÚNG thread đó:
     ...
     Sau đó rẽ theo `auto_resolve_fixed_findings` (đọc từ `meta.json` ở Bước 3 của `pr.md`):
