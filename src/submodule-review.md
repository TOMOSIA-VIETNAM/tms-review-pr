...
Không phải slash command (nằm ngoài `commands/`); `commands/pr.md` nạp file này bằng `Read` CHỈ khi
...
Tới đây, code PR chính đã checkout xong trong 1 worktree ephemeral (`pr.md` Bước 1 mục 1-2), và
...
Trong "Diff đầy đủ" (đã lấy 1 lần ở block Ngữ cảnh của `pr.md`, không fetch lại), tìm đoạn dạng:
...
Tìm trong `body` (description) của PR CHÍNH đã lấy ở block Ngữ cảnh của `pr.md` xem có link PR
...
- **Tìm thấy** → dùng link đó, parse ra `<owner-submodule>/<repo-submodule>/<n-submodule>` (cùng
  cách parse owner/repo/pull_number đã dùng cho PR chính ở `pr.md`).
...
ở `pr.md` Bước 1 mục 4) — KHÔNG gọi `git worktree add` lần nữa cho submodule:

```bash
(cd "<worktree>/<submodule-path>" && gh pr checkout <n-submodule> -R "<owner-submodule>/<repo-submodule>")
```

Cùng ngoại lệ "cấm `cd`" đã nêu ở `pr.md` Bước 1 mục 2 — subshell neo cứng đúng thư mục con này
trong worktree do chính lệnh quản lý, không đổi cwd của phiên chính.

## Bước D — Lấy ngữ cảnh riêng cho PR submodule

Tương tự block "Ngữ cảnh" của `pr.md` nhưng nhắm vào PR submodule — chạy các lệnh sau qua tool
...
Áp dụng lại đúng Bước 2 → Bước 8 của `pr.md` cho diff submodule vừa lấy ở Bước D, với 2 khác biệt
duy nhất:
...
- Memory/template dùng CHUNG thư mục của repo CHÍNH — `notebooks/review/<repo>/` (repo = tên đã
  parse từ PR URL gốc ở đầu `pr.md`). TUYỆT ĐỐI KHÔNG tạo `notebooks/review/<repo-submodule>/`
...
theo đúng schema/quy tắc Bước 9 của `pr.md` (payload `body`/`commit_id`/`comments[]`, xử lý lỗi 422,
...
- `auto_submit_review`/`auto_resolve_fixed_findings` đọc từ CÙNG `meta.json` của repo chính (đã đọc
  ở Bước 3 của `pr.md`) — không hỏi lại, không có bộ cấu hình riêng cho submodule.

Đây là lần POST RIÊNG, KHÔNG tính vào ràng buộc "1 lần POST duy nhất" của Bước 9 `pr.md` (đó là ràng
...
```
### Review PR chính
(tóm tắt kết quả Bước 8-9 của pr.md cho PR chính, kèm link)
