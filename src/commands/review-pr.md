...
**Fetch `headRefOid` NGAY ĐẦU bước này** (không dùng lại giá trị đã lấy ở Ngữ cảnh đầu lệnh — giữa
đó và giờ có thể đã trôi qua nhiều bước, PR có thể nhận commit mới): `gh pr view <url> -R
"<owner>/<repo>" --json headRefOid --jq .headRefOid` (đã nằm trong `allowed-tools`). Gọi giá trị
này là `<commit_id>` — DÙNG LẠI đúng giá trị này cho mọi tier overview dưới (mục "commit đã review")
VÀ cho payload Bước 9 (KHÔNG fetch lại lần 2 ở Bước 9 — cùng 1 giá trị, tránh lệch nếu fetch 2 lần
2 thời điểm khác nhau ra 2 kết quả khác).
...
**Mọi tier dưới đây đều nêu commit đã review** — dev có thể force-push đổi hẳn lịch sử, nêu rõ
review này ứng với commit nào để không mập mờ review cũ có còn khớp code mới nhất hay không. Viết
đúng dạng link (không bọc backtick ra ngoài toàn bộ link — link phải render được, chỉ SHA bên trong
mới ở dạng code):
...
**LGTM 🌟** (commit [`<7 ký tự đầu commit_id>`](...)) — KHÔNG có heading
...
(cảm ơn + commit đã review + hướng dẫn reply, xem cấu trúc dưới) — không thêm câu đánh giá/tổng kết
...
làm việc"), rồi nêu commit đã review (format link trên), rồi 1 câu hướng dẫn reply, xưng "bạn"
(KHÔNG "anh"/"chị"). Sau đó 2-3 câu đánh giá chung + overview title/prefix nếu có.
...
KHÔNG dùng `headRefOid` cũ đã lấy ở Ngữ cảnh đầu lệnh) — Bước 8 fetch giá trị này ngay trước khi
viết overview (đã nêu SHA đó trong overview) chính là để nó đủ mới lúc POST, không cần fetch thêm
lần nữa; fetch lại ở đây chỉ tạo nguy cơ 2 lần fetch 2 thời điểm khác nhau ra 2 giá trị khác, làm
`commit_id` trong payload LỆCH với SHA đã nêu trong overview. `comments[]`
