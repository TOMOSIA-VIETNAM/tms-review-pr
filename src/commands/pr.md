...
- Mỗi object trong `comments[]` gồm `path` + `line` + `"side": "RIGHT"` + `body` — KHÔNG thêm object
  nào thiếu `line` (đó là finding cấp FILE, đã xử lý ở Bước 8, không thuộc payload này).
...
(nghi ngờ đầu tiên: có object nào trong `comments[]` thiếu `line`, hoặc `line` không khớp vị trí
thật trong diff), sửa payload rồi gọi lại ĐÚNG 1 LẦN NỮA. KHÔNG tự tạo review/comment thử nghiệm
...
-f event="COMMENT"` (thay `{review_id}` bằng `id` vừa lấy được). Đây là TOÀN BỘ việc verify cần làm
— không tự thêm bước kiểm tra nào khác (không re-fetch diff để đối chiếu, không liệt kê lại từng
