...
`meta.json` (mặc định `false` nếu thiếu) — dùng lại ở Bước 6 và Bước 9. Giữ lại luôn
`pr_template_paths` (mặc định mảng rỗng nếu thiếu) — dùng lại ở Bước 7.
...
**Kiểm tra title & description PR trước:**
...
- Đọc `title` + `body` (description) của PR đã lấy ở block Ngữ cảnh. Đánh giá: title + description
  ...
  Đây là nhận xét TỔNG QUAN, KHÔNG tính vào 3 mức nghiêm trọng của Bước 8, KHÔNG vào `comments[]` ở
  Bước 9 (không gắn được với 1 file/dòng cụ thể trong diff).
- Kiểm tra prefix theo branch: nếu `headRefName` (tên branch) chứa dạng mã backlog/ticket (vd
  ...
  chứa dạng mã ticket nào → BỎ QUA HOÀN TOÀN kiểm tra này, không ép buộc phải có prefix. Cũng là
  nhận xét TỔNG QUAN, không tính N, không vào `comments[]` — như mục trên.
- **Đối chiếu với PR template checklist của dự án (nếu có) — KHÁC 2 mục trên, mục này CÓ tính vào
  severity.** Nếu `pr_template_paths` (đọc từ `meta.json` ở Bước 3) không rỗng: đọc nội dung (các)
  file tại (các) path đó bằng `Read`, đối chiếu với `body` thật của PR. Tìm dấu hiệu còn sót lại
  chưa điền — vd checkbox `- [ ]` còn chưa tick, hoặc 1 section của template còn nguyên văn bản
  hướng dẫn/placeholder/HTML-comment gốc thay vì nội dung thật của PR. Đây là phán đoán ngữ cảnh,
  KHÔNG có danh sách cứng "mục nào bắt buộc phải điền". Phát hiện ≥1 chỗ chưa điền → gộp thành ĐÚNG 1
  finding TỔNG HỢP (liệt kê các mục còn thiếu trong cùng 1 finding, KHÔNG tách vụn mỗi checkbox
  thành 1 finding riêng) theo khung Vấn đề/Cách fix ở dưới. Finding này XẾP mức **[Nên sửa]**, TÍNH
  vào N ở Bước 8 (vi phạm 1 rule dự án đã tự đặt ra qua PR template, không chỉ là góp ý phong cách
  như 2 mục trên) — là finding cấp **FILE** (không gắn 1 dòng code cụ thể) nên vào body Bước 8 dưới
  mục [Nên sửa], KHÔNG vào `comments[]` ở Bước 9. Nếu `pr_template_paths` rỗng (dự án không có PR
  template) → bỏ qua hoàn toàn mục này, không có gì để đối chiếu, không tạo finding nào.
...
buộc phải đi kèm, vi phạm checklist PR template của dự án...) hay cấp **LINE**
(bug/logic/security/performance tại 1 đoạn code cụ thể). Đây là phán đoán theo ngữ cảnh của agent
lúc review, KHÔNG dùng enum/danh sách cố định để phân loại.
...
- **Đọc thêm codebase ngoài diff là phán đoán của agent, không bắt buộc.** Code PR đã có sẵn trên
  đĩa từ Bước 1 — diff không đủ để đánh giá đúng (vd cần xem hàm được gọi từ file khác, convention
  hiện có của 1 module liên quan) thì tự đọc thêm bằng `Read`/`Grep` trên local; diff đã đủ rõ ràng
  thì không cần chủ động mở rộng đọc toàn bộ codebase.
...
(nếu có, thêm ngay ở đây 2 nhận xét overview-only của Bước 7 — business title/description, prefix
branch-ticket — không tính vào 3 mức bên dưới, không đưa vào comments[] ở Bước 9, vì không gắn với
1 file/dòng cụ thể)
...
cần tính vào N.)
#### [Nên sửa] (N vấn đề)
(tương tự — finding cấp FILE ở mức này; bao gồm cả finding đối chiếu PR template checklist của
Bước 7 nếu có phát sinh)
#### [Đề xuất] (N vấn đề)
...
- Thay `{owner}`, `{repo}`, `{pull_number}` bằng giá trị thật đã parse. Đây là lần gọi `gh api`
  POST DUY NHẤT của cả lệnh — không gọi thêm lần POST review nào khác. Khi có mặt, `event` LUÔN là
  `"COMMENT"` — KHÔNG BAO GIỜ dùng `"APPROVE"` hay `"REQUEST_CHANGES"` (đó là quyết định của con
  người, ngoài phạm vi quy tắc an toàn ở đầu file này).
...
comment để soát, không tạo review test) ngoài 1 lệnh kiểm tra `state` này.
