...
Dù rẽ nhánh nào, giữ lại từ `meta.json`: `auto_submit_review`/`auto_resolve_fixed_findings` (mặc
định `false` nếu thiếu, dùng lại ở Bước 6/9) và `pr_template_paths` (mặc định mảng rỗng nếu thiếu,
dùng lại ở Bước 7).
...
- **Không rỗng** → `Read` `"${CLAUDE_PLUGIN_ROOT}"/src/cases/re-review.md` và làm theo (đề xuất
  lesson từ đồng thuận thread + kiểm tra finding cũ do chính lệnh này để lại đã fix chưa).
- **Rỗng** (PR mới toanh, chưa có comment nào) → bỏ qua hoàn toàn, KHÔNG đọc file đó, sang thẳng
  Bước 7.
...
- **Đối chiếu với PR template checklist của dự án (nếu có) — KHÁC 2 mục trên, mục này CÓ tính vào
  severity.** `pr_template_paths` (đọc từ `meta.json` ở Bước 3) không rỗng → `Read`
  `"${CLAUDE_PLUGIN_ROOT}"/src/cases/pr-template-checklist.md` và làm theo. Rỗng (dự án không có PR
  template) → bỏ qua hoàn toàn, KHÔNG đọc file đó, không tạo finding nào cho mục này.
