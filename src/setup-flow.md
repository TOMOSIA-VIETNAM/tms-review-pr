...
6. Hỏi user **6 câu trong 1 lượt**, ngay trong chat (câu hỏi tự nhiên là đủ, không bắt buộc tool cụ
   ...
   vượt mức thì hỏi chiến lược review trước khi làm (mặc định **`30`** nếu user không chọn). Xử lý
   câu trả lời:
   ...
   - **`auto_submit_review` / `auto_resolve_fixed_findings` / `doctor_schedule` / `review_ci_status`
     / `many_files_threshold`** → ghi nhớ, đưa vào `meta.json` cùng `bootstrapped: true` ở bước 9
     (schema Phần D). `doctor_schedule` thiếu hoặc không parse được → ghi `"1 months"`;
     `review_ci_status` thiếu → ghi `true`; `many_files_threshold` thiếu/không parse được số → ghi
     `30`.
...
9. Ghi nhận vào `notebooks/review/<repo>/meta.json` (tạo file nếu chưa có, giữ nguyên các field
   ...
   `"many_files_threshold": <bước 6, default 30>`, và object `_comments` (ít nhất key
...
- **User config** (Phần A hỏi user lúc bootstrap, đổi lại được qua "đổi cấu hình review"):
  `auto_submit_review`, `auto_resolve_fixed_findings`, `doctor_schedule`, `review_ci_status`,
  `many_files_threshold`. Thiếu ở repo đã bootstrap từ trước → `review-pr.md` Bước 3 tự backfill
...
- **Doctor-detected** (Phần C tự dò lại theo lịch, không phải setting user chọn): `project_docs_found`,
  `templates_copied`, `has_submodules`, `pr_template_paths`. Thiếu vì doctor chưa từng chạy/đang due
  → chờ Phần C chạy lại bình thường, KHÔNG áp dụng backfill của nhóm User config.
...
`has_submodules` detect 1 lần lúc doctor (Phần C bước 5). `review-pr.md` Bước 1 mục 5 đọc field này.
