...
Claude Code **plugin** tên `tms` — 1 slash command duy nhất `/tms:review_pr <PR_URL>` để review PR
...
plugin vào Claude Code rồi gọi `/tms:review_pr <PR_URL>` thật trên 1 repo khác.
...
src/commands/review_pr.md            Slash command DUY NHẤT /tms:review_pr — **thin orchestrator**: mindset +
...
src/stack-detection.md        KHÔNG phải slash command. Bảng mapping đuôi file/path → stack +
                               overlay rule; `review_pr.md` Bước 2 đọc bằng Read
src/setup-flow.md             KHÔNG phải slash command (có ý — xem bên dưới). `review_pr.md` chỉ đọc
...
src/cases/*.md                KHÔNG phải slash command. Logic review-time CÓ ĐIỀU KIỆN — hard gate
                               boolean trong `review_pr.md`, chỉ `Read` khi trigger đúng. Hiện có:
...
**`src/commands/review_pr.md` = thin orchestrator (Bước 0–9).** Validate URL linh hoạt → context
...
POST lỗi hoặc verify lệch → hard gate `post-review.md`. Hành vi ngoài slash command (lesson /
doctor lại) sống ở seed `ALWAYS_RULE.md`, không còn Bước 10 trong `review_pr.md`.
...
**Quy tắc an toàn CRITICAL (đầu `review_pr.md`, trước Bước 0):** lệnh này CHỈ được review + post 1 review
...
tái sử dụng, nhiều lần `/tms:review_pr` chạy đồng thời (cùng PR hay khác PR, cùng hay khác repo) luôn có
...
`review_pr.md`/`src/setup-flow.md` cấm `cd`, cấm tự dò "git root"/"repo thật sự", cấm tự "thông minh" điều hướng
...
với 1 trigger riêng CỦA PR ĐANG REVIEW — `review_pr.md` chỉ `Read` file đó khi trigger đúng, nên nhiều PR
...
- `submodule-review.md` — trigger: `meta.json.has_submodules == true` (detect đúng 1 lần lúc doctor,
  ...
  (lặp lại Bước 2→8 của `review_pr.md` cho diff submodule) — DÙNG CHUNG memory/template của repo CHÍNH
...
Thư mục này CHỦ Ý để MỞ RỘNG: case mới = hard gate boolean + 1 file, không nhét vào `review_pr.md`.

**Lý do bug đã gặp (D — không đưa lại runtime `review_pr.md`):** API 422 "position null" khi trộn comment
...
trực tiếp.** Lần đầu 1 stack xuất hiện trong 1 repo, `review_pr.md` (qua `src/setup-flow.md` Phần B) copy
...
memories/, templates/, ALWAYS_RULE.md, meta.json, worktrees/}` được `/tms:review_pr` tự tạo bên trong
