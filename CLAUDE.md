...
src/commands/pr.md            Slash command DUY NHẤT /review:pr — **thin orchestrator**: mindset +
...
src/cases/*.md                KHÔNG phải slash command. Logic review-time CÓ ĐIỀU KIỆN — hard gate
                               boolean trong `pr.md`, chỉ `Read` khi trigger đúng. Hiện có:
...
**`src/commands/pr.md` = thin orchestrator (Bước 0–9).** Validate URL linh hoạt → context
...
doctor lại) sống ở seed `ALWAYS_RULE.md`, không còn Bước 10 trong `pr.md`.
...
Thư mục này CHỦ Ý để MỞ RỘNG: case mới = hard gate boolean + 1 file, không nhét vào `pr.md`.

**Lý do bug đã gặp (D — không đưa lại runtime `pr.md`):** API 422 "position null" khi trộn comment
...
`/review:pr` (xác nhận trước khi ghi lesson; "doctor lại"). **Không auto-migrate** local đã
