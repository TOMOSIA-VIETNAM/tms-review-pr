...
src/commands/pr.md            Slash command DUY NHẤT /review:pr — CHỈ chứa logic review, không
                               ...
                               (chỉ review + comment, không close/merge/push/xoá branch — cộng thêm
                               đúng 1 review nữa lên PR submodule khi case đó áp dụng, xem Kiến trúc
                               cốt lõi). allowed-tools thu hẹp đúng subcommand cần dùng: `gh pr view`,
                               `gh pr diff`, `gh pr checkout`, `gh api`, `git init`,
                               `git -C notebooks/review:*`, `git fetch`, `git status`, `git show`,
                               `git worktree add notebooks/review/*/worktrees/*` (neo cứng path),
                               `cd notebooks/review/*/worktrees/* && gh pr checkout` (subshell neo
                               cứng đúng thư mục worktree), `git -C notebooks/review/*/worktrees/*
                               submodule update` (cũng neo cứng), `cp`, `mkdir`, `Agent` (doctor song
                               song), `Read`, `Grep`, `Write`, `Edit` — KHÔNG có `gh pr close/merge`,
                               KHÔNG có `git push/branch -D/reset --hard`, KHÔNG còn `git branch`/
                               `git checkout` trần (bỏ vì Bước 1 không còn đổi branch trên pwd chính)
...
src/cases/*.md                KHÔNG phải slash command. Logic review-time áp dụng CÓ ĐIỀU KIỆN theo
                               ...
                               CHỦ Ý để MỞ RỘNG. Hiện có `re-review.md`, `pr-template-checklist.md`,
                               `submodule-review.md` (chi tiết + trigger từng file ở Kiến trúc cốt lõi)
...
`owner/repo/pull_number` từ phần khớp → lấy context qua `gh pr view/diff -R owner/repo` (kèm review
comments cũ của chính PR, fetch 1 lần, dùng ở Bước 6) → tạo 1 `git worktree` ephemeral, checkout code
PR vào đó, và `git submodule update --init --recursive` vô điều kiện (Bước 1; nếu phát hiện submodule
bump kèm link trong description, đọc thêm `src/cases/submodule-review.md`) → detect stack theo đuôi
file/path cho từng file trong diff → thiết lập lần đầu nếu cần (đọc `src/setup-flow.md` có điều kiện)
→ đảm bảo có local template cho từng stack → nạp `src/ALWAYS_RULE.md` LOCAL + memory + template local
→ nếu PR đã có comment cũ, đọc `src/cases/re-review.md` để phát hiện đồng thuận convention mới VÀ
kiểm tra finding cũ do chính lệnh này để lại đã fix chưa → review theo khung 6 mục (đối chiếu thêm PR
template checklist của dự án qua `src/cases/pr-template-checklist.md` nếu repo có template) → định
dạng kết quả → post đúng 1 lần qua `gh api POST .../pulls/{n}/reviews` cho PR chính (có field `event`
hay không tuỳ `auto_submit_review`; cộng thêm đúng 1 lần POST riêng lên PR submodule nếu case đó áp
dụng).
...
tái sử dụng, nhiều lần `/review:pr` chạy đồng thời (cùng PR hay khác PR, cùng hay khác repo) luôn có
...
- `submodule-review.md` — trigger: `meta.json.has_submodules == true` (detect đúng 1 lần lúc doctor,
  ...
  (lặp lại Bước 2→8 của `pr.md` cho diff submodule) — DÙNG CHUNG memory/template của repo CHÍNH
...
memories/, templates/, ALWAYS_RULE.md, meta.json, worktrees/}` được `/review:pr` tự tạo bên trong
