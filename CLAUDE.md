...
commands/pr.md                Slash command DUY NHẤT /review:pr — CHỈ chứa logic review, không
                               ...
                               lần đầu (xem setup-flow.md). allowed-tools: Bash(gh/git/cp/mkdir:*),
...
ALWAYS_RULE.md                Rule cứng global — bản "seed". Lúc bootstrap 1 repo, được `cp` vào
...
**`commands/pr.md`** encode 10 bước tuần tự (Bước 0-9): validate — nhận diện linh hoạt 1 tham chiếu
...
trong diff → thiết lập lần đầu nếu cần (đọc `setup-flow.md` có điều kiện) → đảm bảo có local template
cho từng stack → nạp `ALWAYS_RULE.md` LOCAL + memory + template local → đọc lại comment cũ của chính
...
`pr.md`/`setup-flow.md` cấm `cd`, cấm tự dò "git root"/"repo thật sự", cấm tự "thông minh" điều hướng
...
default English). Bản plugin `${CLAUDE_PLUGIN_ROOT}/ALWAYS_RULE.md` (biến môi trường chuẩn Claude
