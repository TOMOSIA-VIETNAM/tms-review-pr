...
**Team Marketplace:** この GitHub リポジトリを import（Dashboard → Plugins → Team Marketplaces）。Cursor 用 manifest は `.cursor-plugin/marketplace.json`（`source: "./src"`）。インストール後のコマンドは `/review-pr`。

リポジトリを clone してプラグインを改修する場合 → [For development](#for-development) を参照。
...
**並行作業でもブランチを壊す心配なし。** レビューのたびに、PR のコードは専用の [git worktree](https://git-scm.com/docs/git-worktree) へチェックアウトされます — あなたが作業中のリポジトリのブランチ／ワーキングツリーは変更されません。現在のブランチで通常どおりコミット／編集しながら、複数のレビューセッション（同時に複数の PR）を開けます。
...
このリポジトリを clone してプラグイン自体を改修する人向け（レビュー用途だけでインストールするユーザー向けではありません）。実行時の本体は `src/` にあります — ルートの README / `scripts/` / `CLAUDE.md` は install 時にユーザーマシンへはコピーされません。

### Claude Code（ローカル再インストール）

```bash
./scripts/reinstall.sh
```

ローカル marketplace を外して入れ直し、`tms@review-pr` をインストールして現在の `src/` を強制再読み込みします（古いキャッシュ回避）。`claude` CLI が PATH にある必要があります。その後 `/reload-plugins` または新セッションで試してください：
...
`src/` を `~/.cursor/plugins/local/tms` へコピー（**実ディレクトリ** — Cursor は `plugins/local` 外を指す symlink を拒否します）。Cursor を再起動、または **Developer: Reload Window**。コマンドは `/review-pr`。

`src/` を編集するたびにスクリプト再実行 + reload。`gh` が無い／未ログインの場合はスクリプトが警告します。

### レビューフローを変えるとき

- **Source of truth** = `src/commands/review-pr.md`（Claude）。Bước 0–10 を変えたら Cursor adapter `src/cursor/commands/review-pr.md` も合わせて更新（tool map / Shell / prose allowlist）。
- Shared（`setup-flow.md`、`cases/*`、`templates/*`、`ALWAYS_RULE.md`）は `${CLAUDE_PLUGIN_ROOT}` と Claude の tool 名のまま。Cursor は runtime でマップします。
- 新スタック: `src/templates/<stack>.md` + `src/stack-detection.md`（`CLAUDE.md` 参照）。
